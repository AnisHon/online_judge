<template>
  <main class="solution-editor-page common-max-width-page">
    <header class="editor-header">
      ...
      <el-button type="primary" size="large" :icon="Promotion" :loading="isLoading" @click="submit">
        {{ isAdd ? '发布题解' : '保存修改' }}
      </el-button>
    </header>
    ...
    <section class="editor-card">
      <el-form ref="formRef" class="solution-form" size="large" label-position="top" :rules="rules" :model="form">
        <el-form-item class="title-item" prop="title">
          <el-input v-model="form.title" class="title-input" maxlength="100" show-word-limit
        ...
        <div class="form-grid">
          <el-form-item label="关联题目" prop="problemId">
            <div class="problem-select-field">
              <el-input :model-value="selectedProblemText" readonly :title="selectedProblemFullText"
                        placeholder="请选择题目">
                <template #prefix>
                  <el-icon>
                    <Collection/>
          ...
          <el-form-item label="展示设置">
            <div class="visibility-settings">
              <div class="setting-option"><span class="setting-option__icon"><el-icon><Lock/></el-icon></span><span><strong>仅自己可见</strong><small>暂不公开这篇题解</small></span>
                <el-switch v-model="form.private_"/>
              </div>
              <div v-if="hasSolutionEditPermission" class="setting-option"><span
                  class="setting-option__icon setting-option__icon--amber"><el-icon><Top/></el-icon></span><span><strong>置顶题解</strong><small>在题解列表中优先展示</small></span>
      ...
      <div class="content-heading">
        <div><span class="section-index">01</span>
          <div><strong>题解正文</strong><small>支持 Markdown、代码块与数学公式</small></div>
        </div>
        <span class="content-status"><i/>Markdown 编辑模式</span></div>
      <MarkDownEditor v-model="form.content" class="editor"/>
    </section>

    <el-dialog v-model="problemPickerOpen" title="选择关联题目" width="min(980px, 94vw)" append-to-body
               destroy-on-close>
...
import {
  ...
  debouncedAddSolutionAdmin,
  debouncedEditSolution,
  debouncedEditSolutionAdmin,
  getSolution,
  getSolutionAdmin,
  type SolutionForm
} from '@/api/solution'
...
const addSolutionAdmin = debouncedAddSolutionAdmin((success: boolean) => {
  if (success) router.back()
}, finish)
const addSolution = debouncedAddSolution((success: boolean) => {
  if (success) router.back()
}, finish)
const editSolutionAdmin = debouncedEditSolutionAdmin((success: boolean) => {
  if (success) router.back()
}, finish)
const editSolution = debouncedEditSolution((success: boolean) => {
  if (success) router.back()
}, finish)
...
const submit = async () => {
  ...
  let valid = false
  await formRef.value.validate((result, invalidFields?: ValidateFieldsError) => {
    valid = result;
...
.title-input :deep(.el-input__wrapper) {
  padding: 8px 0;
  border-radius: 0;
  box-shadow: 0 1px 0 var(--el-border-color) !important;
...
.solution-form :deep(.el-form-item__label) {
  height: auto;
  margin-bottom: 7px;
...
.problem-select-field {
  min-width: 0;
}

.problem-select-field :deep(.el-input__inner) {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.problem-select-field small {
  display: block;
  margin: 6px 0 0 2px;
  color: var(--el-text-color-secondary);
  font-size: 11px;
...
.setting-option__icon--amber {
  background: color-mix(in srgb, var(--el-color-warning) 13%, var(--el-bg-color));
...
.section-index {
  color: var(--el-color-primary);
  font: 700 11px var(--code-font-family, monospace);
  letter-spacing: .08em;
}

.content-heading strong, .content-heading small {
  display: block;
}
