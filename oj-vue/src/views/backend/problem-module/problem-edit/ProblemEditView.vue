    ...
    <el-scrollbar ref="scrollbarRef" class="problem-editor-scroll">
      <header class="editor-header">
        ...
        <div class="editor-header__actions">
          <span class="save-state"><i :class="{ 'is-ready': !isGetLoading }" />{{ isGetLoading ? '正在读取' : isAdd ? '新建模式' : '已加载原题' }}</span>
...
import {debouncedAddProblem, debouncedAdminGetProblem, debouncedUpdateProblem, dict, ProblemAuth, type ProblemForm, ProblemType} from '@/api/problem';
...
const handleTypeChange = () => { problem.choices.splice(0); problem.cases.splice(0); };
const openCaseManager = () => router.push({name: 'case-edit', params: {problemId: problem.problem.problemId}});
const back = async () => { const name = String(route.name || 'edit-problem'); await router.push({name: 'problem-edit'}); tabStore.removeTab(name); };
const {loading: updateLoading, isLoading: isUpdateLoading, update} = debouncedUpdateProblem(problem, () => {});
const {loading: addLoading, isLoading: isAddLoading, add} = debouncedAddProblem(problem, (id: IdType) => { if (id && id !== 'null') { ElMessage.success('题目已创建'); void router.back(); } });
const submit = async () => {
  ...
  if (isAdd.value) { addLoading(); add(); } else { updateLoading(); update(); }
};
const {isLoading: isGetLoading, loading: getLoading, get} = debouncedAdminGetProblem(problemId.value, (data) => { __.assign(problem.problem, data.problem); if (data.ojProblem) __.assign(problem.ojProblem, data.ojProblem); problem.cases.splice(0, problem.cases.length, ...(data.cases || [])); problem.choices.splice(0, problem.choices.length, ...(data.choices || [])); });
if (!isAdd.value) { getLoading(); get(); }
watch(() => problem.problem.type, (type, previous) => { if (isAdd.value && previous !== undefined && type !== previous) handleTypeChange(); });
