<template>
  <div class="contest-container">
    ...
    <div class="contest-list" v-loading="isLoading">
      <el-card v-for="item of list" shadow="hover" class="card">
        <el-row>
          ...
          <el-col :span="6" style="display: flex; justify-content: center; align-items: center;">
            ...
            <el-button style="margin-left: auto" type="success" v-else>
              参加比赛
            </el-button>
          </el-col>
        </el-row>
...
import {type ContestView, debouncedGetContest} from "@/api/contest";
...
const joinContest = (start: string, end: string, id: number) => {
  const currTimeStamp = dayjs().unix();
  const startTimeStamp = dayjs(start).unix();
  const endTimeStamp = dayjs(end).unix();
