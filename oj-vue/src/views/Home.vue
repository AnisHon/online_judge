    ...
    <section class="home-grid">
        ...
        <home-section-card title="最近题解" icon="EditPen" more @show-more="go('solutions')">
          <div class="home-list" v-loading="loading">
            <button v-for="item in solutions" :key="item.solutionId" class="home-row home-row--solution" type="button" @click="go('solution', item.solutionId)">
              <span class="home-row__avatar-link" role="link" tabindex="0" @click.stop="go('profile', item.userId)" @keydown.enter.stop="go('profile', item.userId)"><avatar class="home-row__avatar" :user-id="item.userId" /></span>
              <span class="home-row__content">
      ...
      <home-section-card class="ranking-card" title="排行榜" icon="Histogram">
        ...
        <div v-loading="loading" class="ranking-list">
          <button v-for="(item, index) in ranks" :key="item.userId" type="button" class="ranking-row" :class="`ranking-row--${index + 1}`" @click="go('profile', item.userId)">
...
.ranking-intro { display: flex; align-items: center; justify-content: space-between; padding: 14px 0 11px; }.ranking-intro strong, .ranking-intro span { display: block; }.ranking-intro strong { font-size: 13px; }.ranking-intro span { margin-top: 3px; color: var(--el-text-color-secondary); font-size: 11px; }.ranking-intro .ranking-count { margin-top: 0; color: var(--el-color-warning); font: 700 11px var(--code-font-family, monospace); }.ranking-list { min-height: 120px; }.ranking-row { display: flex; min-width: 0; width: 100%; align-items: center; gap: 9px; min-height: 58px; padding: 0; border: 0; border-top: 1px solid var(--el-border-color-lighter); color: inherit; background: transparent; text-align: left; cursor: pointer; }.ranking-row:hover { background: var(--el-fill-color-lighter); }.ranking-position { display: grid; width: 23px; flex: 0 0 auto; place-items: center; color: var(--el-text-color-placeholder); font: 700 11px var(--code-font-family, monospace); }.ranking-position .el-icon { color: var(--el-text-color-placeholder); font-size: 16px; }.ranking-row--1 .ranking-position .el-icon { color: #e5a819; }.ranking-row--2 .ranking-position .el-icon { color: #9ba8b6; }.ranking-row--3 .ranking-position .el-icon { color: #bd7c52; }.ranking-avatar { width: 34px; height: 34px; }.ranking-avatar :deep(.avatar__image) { width: 34px; height: 34px; }.ranking-user { display: flex; min-width: 0; flex: 1; flex-direction: column; gap: 2px; }.ranking-user strong, .ranking-user small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.ranking-user strong { font-size: 12px; font-weight: 650; }.ranking-user small { color: var(--el-text-color-secondary); font-size: 10px; }.ranking-points { flex: 0 0 auto; text-align: right; }.ranking-points strong { display: block; color: var(--el-color-primary); font: 750 15px var(--code-font-family, monospace); }.ranking-points small { display: block; margin-top: 1px; color: var(--el-text-color-secondary); font-size: 10px; }.home-row__avatar-link { display: inline-flex; flex: 0 0 auto; cursor: pointer; }
