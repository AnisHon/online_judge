...
- Q-01：拆分题单行删除与批量删除 handler；行删除只提交当前行，批量删除使用 selection 快照。
- Q-02：题单排序接口复用现有 `problem_list.update_time` 做乐观锁，事务内 `for update` 锁定版本、校验完整题目集合并推进版本；前端保存成功后重新读取服务端顺序和版本，冲突时回滚到服务端状态。
- Q-03：保留鼠标拖动并增加上下移动按钮、键盘操作和拖动前/后插入反馈，移动端不依赖 HTML5 拖放也可调整顺序。
- Q-04/Q-06：题目保存改为点击时生成 payload 并等待请求；题目 ID 和题单 ID 都通过响应式 route 监听，旧请求不会覆盖新页面。
- Q-05：新增题型切换确认和按题型草稿缓存，返回/切路由时增加未保存确认。
- Q-07：分数保存使用行级 saving 状态和请求序列，失败恢复最后一次已提交分数。
- Q-08/Q-11：标签差异继续使用稳定 ID 集合；上传增加 JSON 类型校验和纯文本/非 JSON 错误安全解析。
- Q-09/Q-10：编辑器滚动容器补充宽度边界和横向溢出隔离；答案/测试点编辑器共用 `EditorToolbar`、`EditorEmptyState`，移除重复工具栏和空状态样式。
- Q-12：编辑页返回目标固定为题目管理路由，保留动态编辑页 tab 清理逻辑。

验证：`pnpm run type-check`、`pnpm run build-only`、`JAVA_HOME=/usr/lib/jvm/java-11-temurin-jdk mvn -pl problem-service -am -DskipTests compile` 均通过。
