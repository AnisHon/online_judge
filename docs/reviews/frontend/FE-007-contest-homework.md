...
- 本文先记录 review 问题，再记录本次已执行的修复结果；未提交 Git，未清理工作区中的其他改动。

## 2. 执行结论

本次已完成 FE-007 的主要修复。比赛/作业列表不再从共享可变表单推断异步响应，活动详情的活动信息、答题状态、题目列表可以独立降级，路由参数变化会清理并重新加载当前活动，交卷结果会区分取消、业务失败、网络异常和成功。活动状态和时间展示也增加了响应式时钟、非法时间兜底和移动端抽屉式侧栏。

本次覆盖了 FE-007-01～20 中的核心问题：加入/密码确认防重、列表和题目列表错误态、请求序号保护、活动路由复用、交卷状态机、时间状态、侧栏拖动生命周期、窄屏布局、标题截断、键盘语义和空态闪烁均已处理。后端仍保留最终的活动权限、开始/结束时间和交卷幂等校验，前端状态只负责及时反馈，不替代服务端裁决。
...
- 逐行检查并修改了 `ActivityList`、`ContestProblems`、`ActivityResizablePanel`、比赛/作业入口、活动 API、题单题目 API 和活动时间工具。
- 检查了活动列表加入、私有密码、白名单、题目导航、活动概况、交卷刷新和 DetailProblem 的活动复用路径。
- 清理了活动页重复 CSS 和死选择器，补充了活动 metadata、请求错误态、题目状态的读屏文本、侧栏键盘调整和移动端抽屉布局。
- 将活动列表和题目列表前台路径切换到直接 Promise API；保留原有后台使用的 debounce API 兼容入口，但不再把失败转换为空数组。
...
- 需要浏览器实际验证快速连续点击不同活动、慢网络/403、同路由切换活动 ID、开始/结束边界、交卷重复点击、320～1440px、暗色模式、键盘拖动和移动端侧栏焦点。
- 当前项目没有覆盖这些场景的活动组件测试；后续可补充请求序号、时间状态、交卷状态机和侧栏拖动的单元测试/浏览器测试。

## 7. 本次修改内容

- 活动列表使用活动 ID 快照处理加入、查询已加入、私有密码和白名单流程，加入/确认按钮有独立 loading 与重复提交保护。
- 活动列表和题目列表增加 loading/success/empty/error 语义；失败不会再被转换为空数组，提供页面级重试并保留当前请求快照。
- 活动详情拆分活动信息、答题状态和题目列表请求，路由 ID 变化时清除旧活动、题目、当前选择和侧栏状态，旧响应不会污染新活动。
- 交卷流程只把 MessageBox 的取消显示为“已取消提交”，接口失败显示真实错误；成功后刷新状态和题目完成度，并用提交锁防止重复交卷。
- 活动时间统一由工具函数处理，增加响应式时钟、合法性判断、错误时间显示和年/月/日/小时持续时间格式。
- `ActivityResizablePanel` 增加 pointerup/pointercancel/lostpointercapture/unmount 清理、键盘步进、ARIA 数值语义；窄屏切换为覆盖式可隐藏侧栏，不再强行保留桌面百分比栏宽。
- 活动题目导航补充 `aria-current`、完成状态文本、标题截断；顶部题目栏操作区固定收缩，活动列表卡片在移动端改为稳定 grid 布局。
- 未修改数据库，未操作部署环境，未提交 Git。
